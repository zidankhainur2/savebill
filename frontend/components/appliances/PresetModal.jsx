...
import { X, Plug, Zap } from 'lucide-react';
...
export default function PresetModal({ isOpen, onClose, onSelectPreset }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-3xl max-w-lg w-full p-6 max-h-[80vh] flex flex-col shadow-lg border border-black/10">
        <div className="flex items-center justify-between pb-3 border-b border-black/5">
          <div className="flex items-center gap-2">
            <Plug className="w-4 h-4 text-emerald-800" />
          ...
          {isLoading ? (
            ...
            presets.map((preset) => (
              <button
                ...
                className="text-left p-3.5 rounded-2xl border border-black/5 bg-[#F6F7F2] hover:bg-white hover:border-black/20 hover:shadow-xs transition-all group"
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="font-bold text-xs text-emerald-950 group-hover:text-emerald-800">
                    {preset.name}
                  </span>
                  <span className="text-[10px] font-medium uppercase tracking-wide bg-emerald-900/10 px-2 py-0.5 rounded-full text-emerald-900">
                    {preset.category}
                  </span>
                </div>
                <div className="flex items-center gap-1 text-xs text-emerald-900/60">
